...
def get_api_key(request: Request, api_key: str = Depends(api_key_header)):
    if not api_key or api_key != request.app.state.api_token:
...
" + injection, 1)
    ...
    async def api_autonomous(request: Request, api_key: str = Depends(get_api_key)):
    ...
    @limiter.limit("10/minute")
    async def api_control(request: Request, api_key: str = Depends(get_api_key)):
        body = await request.json()
        with state.lock:
            if "active" in body:
    ...
    @app.get("/api/audit")
    @limiter.limit("10/minute")
    async def api_audit(request: Request, api_key: str = Depends(get_api_key)):
    ...
    @limiter.limit("10/minute")
    def api_drone_connect(req: DroneConnectRequest, request: Request, api_key: str = Depends(get_api_key)):
        """
    ...
    @limiter.limit("30/minute")
    def api_drone_command(req: DroneCommandRequest, request: Request, api_key: str = Depends(get_api_key)):
        """
